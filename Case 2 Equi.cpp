...
const int DISTANCE[4] = {0, 20, 33, 47};
const int SERVINGTIME[2] = {27, 30};
const int WORK[2] = {425, 430};
const int WASH = 28;
...
int main()
{
    double* final = manySimulations(100);
    for (int i=0; i<8; i++)
    {
        cout << "CNC";
        cout << i + 1;
...
double* manySimulations(int num)
{
    ...
    for (int i=0; i<8; i++)
    {
        alpha[i] = (rand() % 500000 + 1) / double(500001);
        beta[i] = (rand() % 500000 + 1) / double(500001);
        kappa[i] = 0;
    }
    double M[7][7];
    for (int i=0; i<7; i++)
