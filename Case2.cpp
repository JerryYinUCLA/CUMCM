...
int main()
{
    double* final = manySimulations(200);
    for (int i=0; i<8; i++)
    {
        cout << "CNC";
        cout << i + 1;
...
double* Simulation(double alpha[8], double beta[8], double kappa[8], bool last)
{
    ...
    for (int time = 0; time<28800;)
    {
        ...
        for (int i=0; i<8; i++)
        {
            readyA[i] = CNCdone[i] && (knives[i] == 'A');
            if (readyA[i])
                hasReadyA = true;
            readyB[i] = CNCdone[i] && (knives[i] == 'B');
            if (readyB[i])
                hasReadyB = true;
        }
        if (num % 2 == 1)
        {
            if (hasReadyA)
            {
                ...
                int duration = DISTANCE[move] + SERVINGTIME[nextToServe % 2];
